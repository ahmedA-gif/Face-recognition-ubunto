...
def _pipeline_worker(source_override: Optional[str] = None):
    """Run pipeline in background thread, 24/7 auto-restart on stream end."""
    ...
    try:
        ...
        _pipeline_status["source"] = str(source_override or cfg["camera"]["source"])

        from src.pipeline.runner import run_pipeline

        def on_frame(vis, counts):
            _set_frame(vis)
            _pipeline_status["frames"] = _pipeline_status.get("frames", 0) + 1
            _pipeline_status["events"] = counts.get("entry", 0) + counts.get("exit", 0)
        ...
        while retry_count < max_retries:
            ...
            print(f"[WebPipeline] Starting pipeline (attempt {retry_count + 1})", flush=True)
            try:
                result = run_pipeline(
                    cfg,
                ...
                _pipeline_status["frames"] = _pipeline_status.get("frames", 0) + result.frames_processed
                _pipeline_status["events"] = result.events_count
                print(f"[WebPipeline] Stream ended: frames={result.frames_processed}", flush=True)
            except Exception as e:
                print(f"[WebPipeline] Error: {e}", flush=True)

            if _pipeline_stop_event.is_set():
                break

            retry_count += 1
            wait_sec = min(5 + retry_count * 2, 30)
            print(f"[WebPipeline] Restarting in {wait_sec}s...", flush=True)
            _pipeline_status["error"] = f"Stream ended, restarting in {wait_sec}s..."
...
def dashboard():
    ...
    try:
        all_recent = store.recent(limit=200)
        events_today = [e for e in all_recent if e.get("date") == today]
    except Exception:
        events_today = []
    try:
...
def events():
    store = _get_events_store()
    try:
        all_events = store.recent(limit=200)
    except Exception:
        all_events = []
...
def snapshots():
    store = _get_events_store()
    try:
        all_events = store.recent(limit=500)
    except Exception:
        all_events = []
    snap_events = [e for e in all_events if e.get("snapshot_path")]
    snapshots_dir = str(_snapshots_dir)
    return render_template("snapshots.html", events=snap_events, snapshots_dir=snapshots_dir)
...
def api_pipeline_start():
    global _pipeline_thread, _pipeline_stop_event, _pipeline_running
